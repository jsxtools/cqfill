export const cqfill = (
	(
		...
		getCSSRules = (cssParentRule) =>
			slice.call(cssParentRule.cssRules || []),
			...
			const walkCssParent = (cssParentRule) => {
				...
				for (const cssRule of getCSSRules(cssParentRule)) {
					...
					const hasInlineSizeContainment =
						...
						(hasSizeContainment ||
							containValues.includes('inline-size'))
					const hasBlockSizeContainment =
						hasLayoutContainment &&
						(hasSizeContainment ||
							containValues.includes('block-size'))
		...
		polyfillContainerQueries = (root, styleSheet) => {
			...
			const walkCssParent = (cssParentRule) => {
				...
				for (const cssRule of getCSSRules(cssParentRule)) {
					/** @type {string} */
					const mediaText = cssRule.media
						? cssRule.media.mediaText
						: ''

					const hasContainerQueryPolyfill =
						mediaText.indexOf('@container') === 0 ||
					...
					if (hasContainerQueryPolyfill) {
						...
						if (containerQueryMatches) {
							...
							const cssRuleIndex = getCSSRuleIndexOf(
								cssParentRule,
								cssRule
							)
							const cssPolyfillGroup = insertCssRule(
							...
							for (const cssInnerRule of getCSSRules(cssRule)) {
								...
								const cssInnerRuleSelectorText =
									cssInnerRule.selectorText

								if (cssInnerRuleSelectorText) {
									const cssInnerRuleBlock =
										cssInnerRule.cssText.slice(
			...
			const onFrame = () => {
				...
				if (numberOfStyleSheets !== lastNumberOfStyleSheets) {
					while (lastNumberOfStyleSheets < numberOfStyleSheets) {
						const styleSheet =
							styleSheets[lastNumberOfStyleSheets++]
